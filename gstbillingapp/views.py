...
def invoice_create(request):
    ...
    if request.method == 'POST':
        ...
        invoice_data = request.POST
        invoice_data_processed = invoice_data_processor(request, invoice_data)
        ...
        if invoice_data_processed is None:
            # Error occurred, display error message and redirect back to create invoice page
...
def get_inventory_stock(request):
    product_id = request.GET.get('product_id')
    stock = Inventory.objects.filter(product_id=product_id).values('current_stock').first()

    return JsonResponse({'stock': stock['current_stock']})
...
def check_stock(request):
    ...
    try:
        ...
        stock_available = inventory.current_stock > 0
...
def inventory_json_view(request):
    inventory_data = []
...
def get_stock_view(request):
    ...
    for inventory in inventories:
        stock_data.append({
            'product_name': inventory.product.product_name if inventory.product else '',
            'current_stock': inventory.current_stock
        })
        print(stock_data)
...
def check_inventory(request):
    if request.method == 'POST':
        ...
        if not available:
            response_data['current_stock'] = inventory.current_stock
