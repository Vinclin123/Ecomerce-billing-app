...
def invoice_data_processor(request, invoice_post_data):
    print(invoice_post_data)
    processed_invoice_data = {}
    data = invoice_post_data['invoice-total-amt-with-discount']
    ...
    processed_invoice_data['invoice_total_amt_without_gst'] = 0.0
    processed_invoice_data['invoice_total_amt_sgst'] = 0.0
    processed_invoice_data['invoice_total_amt_cgst'] = 0.0
    processed_invoice_data['invoice_total_amt_igst'] = 0.0
    processed_invoice_data['invoice_total_amt_with_gst'] = 0.0
    processed_invoice_data['invoice_total_amount_after_discount'] = data
    ...
    for idx, product in enumerate(invoice_post_data['invoice-product']):
        if product:
            print(idx, product)
            item_entry = {}
            discount_amount = float(invoice_post_data.get('invoice-discount-amount', 0.0))
            ...
            try:
                inventory = Inventory.objects.get(product__product_name=product)
                
               
               
                #if inventory.current_stock >= item_entry['invoice_qty']:
                if inventory.current_stock >= item_entry['invoice_qty']:
                    
        # Sufficient stock available
                    quantity_to_bill = min(item_entry['invoice_qty'], inventory.current_stock)
                    item_entry['invoice_qty'] = quantity_to_bill
                    # Sufficient stock available
                    item_entry['available_in_inventory'] = True

                    # Recalculate the discounted amount for the product
                    discounted_amount = item_entry['invoice_amt_with_gst'] - discount_amount
                    item_entry['invoice_amt_with_gst'] = discounted_amount

                    processed_invoice_data['items'].append(item_entry)

                    # Update invoice totals for available products
                    processed_invoice_data['invoice_total_amt_without_gst'] += item_entry['invoice_amt_without_gst']
                    processed_invoice_data['invoice_total_amt_sgst'] += item_entry['invoice_amt_sgst']
                    processed_invoice_data['invoice_total_amt_cgst'] += item_entry['invoice_amt_cgst']
                    processed_invoice_data['invoice_total_amt_igst'] += item_entry['invoice_amt_igst']
                    processed_invoice_data['invoice_total_amt_with_gst'] += discounted_amount
                    
                else:
                    # Insufficient stock
                    #print(f"Product '{product}' is out of stock")
                    messages.error(request, f"Product '{product}' is out of stock.The available stock is'{inventory.current_stock}' please check and Rebill")
                    
                    
            except Inventory.DoesNotExist:
                # Product not found in inventory
                #print(f"Product '{product}' does not exist in inventory")
                messages.error(request, f"Product '{product}' does not exist in inventory '{inventory.current_stock}")
